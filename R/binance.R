...
binance_ping <- function() {
    res <- binance_query(endpoint = '/api/v3/ping')
...
binance_klines <- function(symbol, interval, limit, start_time, end_time) {
    ...
    klines <- binance_query(endpoint = 'api/v3/klines', params = params)
...
binance_ticks <- function(symbol, from_id, start_time, end_time, limit) {
    ...
    ticks <- binance_query(endpoint = 'api/v3/aggTrades', params = params)
...
binance_trades <- function(symbol, limit) {
    ...
    trades <- binance_query(endpoint = 'api/v3/trades', params = params)
...
binance_depth <- function(symbol, limit) {
    ...
    depth <- binance_query(endpoint = 'api/v3/depth', params = params)
...
binance_ticker_all_prices <- function() {
    ...
    prices <- binance_query(endpoint = 'api/v3/ticker/price')
...
binance_ticker_all_books <- function() {
    ...
    books <- binance_query(endpoint = 'api/v3/ticker/bookTicker')
...
binance_ticker_24hr <- function(symbol) {
    ...
    if (!missing(symbol)) {
        params <- list(symbol = symbol)
        prices <- binance_query(endpoint = 'api/v3/ticker/24hr', params = params)
        prices <- as.data.table(prices)
    } else {
        prices <- binance_query(endpoint = 'api/v3/ticker/24hr')
        prices <- rbindlist(prices)
    }
...
binance_coins <- function() {
    sort(unique(sub('(BTC|ETH|BNB|USDT|TUSD|PAX|USDC|XRP|USDS|USD4||USD|BUSD)$', '', binance_symbols())))
}
...
#' }
binance_cryptowithdrawals <- function(coin, limit, start_time, end_time) {
  
  params <- list(coin   = coin)
  ...
  cryptowithdrawals <- binance_query(endpoint = '/sapi/v1/capital/withdraw/history', params = params,  sign = TRUE)
  
  assignInMyNamespace('BINANCE_WEIGHT', 0)
...
#' Get crypto deposits from Binance.US
#' @param coin string
#' @param limit optional int
#' @param start_time optional POSIX timestamp
#' @param end_time optional POSIX timestamp
...
#' }
binance_cryptodeposits <- function(coin, limit, start_time, end_time) {
  ...
  cryptodeposits <- binance_query(endpoint = '/sapi/v1/capital/deposit/hisrec', params = params,  sign = TRUE)
  
  assignInMyNamespace('BINANCE_WEIGHT', 0)
  
  cryptodeposits <- bind_rows(cryptodeposits)
  
  
}


#' Get USD withdrawals from Binance.US
#' @param start_time optional POSIX timestamp
#' @param end_time optional POSIX timestamp
#' @return data.table
#' @export
...
#' }
binance_fiatwithdrawals <- function(start_time, end_time) {
  ...
  fiatwithdrawals <- binance_query(endpoint = '/sapi/v1/fiatpayment/query/withdraw/history', params = params,  sign = TRUE)
  
  assignInMyNamespace('BINANCE_WEIGHT', 0)
  
  fiatwithdrawals <- as.data.frame(do.call(rbind, fiatwithdrawals$assetLogRecordList))
  
}


#' Get USD Deposits from Binance.US
#' @param start_time optional POSIX timestamp
#' @param end_time optional POSIX timestamp
#' @return data.table
#' @export
...
#' }
binance_fiatdeposits <- function(start_time, end_time) {
  ...
  fiatdeposits <- binance_query(endpoint = '/sapi/v1/fiatpayment/query/deposit/history', params = params, sign = TRUE)
  
  assignInMyNamespace('BINANCE_WEIGHT', 0)
  
  fiatdeposits <- as.data.frame(do.call(rbind, fiatdeposits$assetLogRecordList))
}
